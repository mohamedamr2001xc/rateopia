...
if(isset($_POST['submit'])){
    ...
    if(empty($nationalid) || empty($email) ){
        ...
        $sql="SELECT National_id FROM citizen WHERE National_id= $nationalid";
        $result=mysqli_query($conn,$sql);
        $row=mysqli_fetch_assoc($result);
        if($row['National_id']==""){
            header("Location: ../change.php?error=nationaliddeosntexist");
        }
        $sql2="SELECT Email FROM citizen WHERE National_id= $nationalid";
        ...
        if($row2['Email']!=$email){
            ...
            try{
                ...
                echo"<br>";
                echo $_SESSION['National_id']."555555";
                } catch (Exception $e) {
