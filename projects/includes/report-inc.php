...
if (isset($_POST['submit'])) {
    ...
    $inctype = $_POST['inc_type'];
    $lat = $_POST['lat'];
    $lng = $_POST['lng'];
    if (empty($message) || empty($inctype) || empty($lat) || empty($lng)) {
        $sql = "DELETE from reports WHERE report_info=''";
        mysqli_query($conn, $sql);
        header("Location: report_submit.php?error=emptyfields&subject=$national_id");
        exit();
    }
    $sql = "SELECT * From incidents_postive WHERE Namee='$inctype' ";
    $result = mysqli_query($conn, $sql);
    if (mysqli_num_rows($result) > 0) {
        $_SESSION['inc_type'] = $_POST['inc_type'];
        $_SESSION['message'] = $_POST['message'];
        $_SESSION['lat'] = $_POST['lat'];
        $_SESSION['lng'] = $_POST['lng'];
        $_SESSION['Files'] = $_FILES['my_image'];
        $sql = "DELETE from reports where report_info=''";
        mysqli_query($conn, $sql);
        if ($_SESSION['csrf'] != $_POST['csrf']) {
            header("Location: logout.php");
        }
        header("Location: report-inc3.php");
        exit();
    }
    ...
    if (!is_numeric($latcheck) || !is_numeric($lngcheck)) {
        ...
        $sql = "SELECT incident_weight From incidents_normal where Namee='$inctype'";
        $result = mysqli_query($conn, $sql);
        ...
        if ($errornoofreports >= 120 || $errorsameuser >= 1 || $noofreportssixmonth >= 3) {
            ...
            $accusedreputation = $accusedrating / 5;
